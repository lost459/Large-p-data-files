function [] = G2_General( D, p, lam, q, tau_end, n_stps )
...
%%% function.
    ...
    % MPS calculation of g2(tau).
    R_flat = reshape(B3*rho_ss*ctranspose(B3),[D^2,1]) / N1;
    ...
    G2 = L1_flat * kron(conj(B3),B3) * expv( 1.0, 0*L+SmallC*I_D2, R_flat, tol, D ) / N1;

    M = [full(G2) D p lam q 0];
    dlmwrite('G2.txt', M, 'Delimiter', '\t', '-append', 'precision', 16);

    for ii = 1:n_stps

        ii/n_stps

        G2 = L1_flat * kron(conj(B3),B3) * expv( 1.0, ii*tau_end*L/n_stps+SmallC*I_D2, R_flat, tol, D ) / N1;

        M = [full(G2) D p lam q ii*tau_end/n_stps];
        dlmwrite('G2.txt', M, 'Delimiter', '\t', '-append', 'precision', 16);
